...
#     """
#     Pipeline: grayscale → CLAHE → (optional binarize) → medianBlur.
#     Input:  BGR or grayscale image (post crop/deskew).
#     Output: clean grayscale image by default; pure black/white if binarize.enabled=True.
#     """
#     if not cfg.enabled:
#         return image
...
#     # CLAHE — improves contrast on yellowed/faded paper
...
#     logger.debug(
#         f"CLAHE: clip_limit={cfg.clahe.clip_limit}, tile_grid={cfg.clahe.tile_grid_size}"
#     )

#     # Optional binarization — only when caller explicitly enables it
...
#         )
#         logger.debug(
#             f"Binarize: block_size={cfg.binarize.block_size}, C={cfg.binarize.c_constant}"
#         )

#     # Median blur — removes salt-and-pepper noise on old documents
#     if cfg.denoise.enabled:
#         result = cv2.medianBlur(result, cfg.denoise.kernel_size)
#         logger.debug(f"Denoise: medianBlur kernel={cfg.denoise.kernel_size}")
