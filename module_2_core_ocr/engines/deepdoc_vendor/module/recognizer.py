...
class Recognizer:
    ...
    def postprocess(self, boxes, inputs, thr):
        ...
        return [{
            # LƯỚI AN TOÀN: Nếu ID nằm ngoài danh sách, đặt tên nó là "unknown"
            "type": self.label_list[int(class_ids[i])].lower() if int(class_ids[i]) < len(self.label_list) else f"unknown_{int(class_ids[i])}",
